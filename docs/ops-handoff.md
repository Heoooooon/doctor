...
- **2026-09-17 `https://egundc.com`을 서치어드바이저에 등록했다**(계정: 서울이건치과의원 / seoul2gun).
  그 전까지 사이트 목록이 비어 있어 네이버 쪽에는 사이트맵 제출·수집 현황 계기판이 없었다.
- 소유확인 방법으로 **HTML 태그**를 선택하면 되고, 필요한 메타 태그는 이미 운영에 배포돼 있다
  (`62dc81a6f5676827f6b1f38fd85a68bd8bc5978b`).
- **남은 절차는 사람이 해야 한다.** 네이버가 자동화 방지 캡차를 요구하므로 브라우저에서 직접
  캡차를 입력하고 `소유확인`을 눌러야 한다. 간단체크 도구도 같은 캡차를 요구하며
  로그인 계정 기준 일간 10회 제한이 있다.
- 소유확인이 끝나면 `요청 > 사이트맵 제출`에 `https://egundc.com/sitemap.xml`을 넣는다.
