...
## 현재 상태 — 구 사이트 주소 404 리다이렉트 정리

**2026-09-21 커밋 `acf6261`을 `./scripts/deploy-vps.sh`로 반영했다.** 직전 릴리스는 `59d441c`이며
백업은 `/var/lib/seoulegundc-deploy/backups/1789997357206.6fa8c5aa-0a5e-4e65-b80e-363369b0cae9.tar.gz`에 있다.

- Search Console 미색인 49건 중 **404가 21건**이었고, 그 대부분이 구 사이트의 데스크톱·**모바일 미러(`/m/*`)**
  주소였다. 추측이 아니라 **nginx 로그에서 검색봇이 실제로 요청한 404 경로를 전수로 뽑아** 규칙을 만들었다.
  구 구조는 `/intro*`·`/counsel*`·`/treatment*`·`/orthodontics/*`·`/implant/*`·`/community/*`와 그것을
  그대로 미러한 `/m/*` 두 벌이다.
- **함정:** `{ source: '/implant/:path*' }`는 `:path*`가 빈 경로까지 매칭해 `/implant` 자신이 자기 자신으로
  308을 돌려 **무한 리다이렉트**가 된다. 목적지와 소스의 첫 세그먼트가 같으면 반드시 `:path+`를 쓴다.
  회귀 검사(실제 페이지 200 확인)에서 잡혔다.
...
- **빙 소유확인은 이미 끝난 상태였다.** 빙 화면 문구: *"was imported from Google Search Console.
  Since it is already verified on Google Search Console, there is no verification code needed on Bing
...
**2026-09-21 커밋 `59d441c`를 `./scripts/deploy-vps.sh`로 반영했다.** 직전 릴리스는 `833d080`이며
백업은 `/var/lib/seoulegundc-deploy/backups/1789991672013.69d66fa0-de35-4e05-ae88-14977fecca6a.tar.gz`에 있다.

- 빙 웹마스터 도구가 XML 파일 방식 소유확인을 요구해 다운로드 원문을 그대로 `public/BingSiteAuth.xml`에 두었다.
  기존 `msvalidate.01` 메타 태그(`data/tracking.ts`의 `bingSiteVerification`)는 같은 값 `0F6CFB59E2…`로 유지했다.
  파일과 메타 태그를 함께 둬도 충돌하지 않는다.
- **삭제 금지:** `public/BingSiteAuth.xml`. 운영 URL `https://egundc.com/BingSiteAuth.xml`은 200,
  `Content-Type: application/xml`이며 본문 SHA-256
  `0683892848b21c3bb18a66bd79463e9cf0f2d5a68b8d61b90d3e0ff4d82ce9b5`가 빙 다운로드 원문과 일치한다.
  `www.egundc.com` 요청은 308로 대표 도메인에 모인다.
- 배포 검증: 로컬 테스트 150/150, 배포 종료 코드 0, `/__release.txt` 커밋 `59d441c`.
- **남은 일:** 빙 웹마스터 도구에서 소유확인 버튼을 눌러 검증을 마쳐야 한다. 파일 배치만으로는 확인되지 않는다.

## 직전 상태(2) — 메타 태그 정리와 네이버 서치어드바이저 등록

**2026-09-17 PR #5(병합 커밋 `41ca139`)를 같은 배포 절차로 반영했다.**
운영 최신 커밋은 `41ca139`이며, 직전 릴리스(`a110549`)와 백업은 보존돼 있다.

- `app/layout.tsx`의 og:image가 존재하지 않는 `/images/logo/egun-logo%20(1).png`를 가리켜
  홈을 포함한 13개 페이지의 대표 이미지가 404(HTML 반환)였다. 실제 파일 `egun-logo.png`(1000x400)로 고쳤다.
- 레이아웃 openGraph를 상속해 12개 페이지가 og:url을 홈 주소로, og:image를 공용 로고로 내보내던 것을
  `lib/page-metadata.ts`의 `pageMetadata()`로 정리했다. 경로·제목·설명·이미지를 넘기면
  canonical·og:*·twitter:*를 한 번에 만든다. **새 페이지를 추가할 때 이 헬퍼를 쓴다.**
- 홈 description과 og:description을 네이버 간단체크 기준(80자 이내)에 맞춰 97자 → 54자로 줄였다.
  하위 페이지는 구글 쪽 이점을 위해 80자를 넘겨 두었다(네이버 간단체크는 사이트 대표 설명을 본다).
- 네이버 소유확인 코드는 `data/tracking.ts`에서 **배열**로 관리한다. 계정마다 코드가 달라
  하나를 지우면 그 계정의 소유확인이 풀리므로 기존 코드와 새 코드를 함께 렌더링한다.
- 배포 검증: 두 도메인 스모크 `passed: true`(검사 173·자산 168·공개 칼럼 18),
  13페이지 전수에서 og:url이 자기 주소·og:image가 페이지별 고유 이미지이며 모두 실제 이미지로 200 응답.

- 네이버 가이드 기준 37페이지 자체 전수검사 통과(2026-09-17): title·description 존재·고유·상호 불일치,
  og:title·description·image·url·type 전부 존재, og:url 모두 자기 주소, og:image 36종 전부
  권장 규격(150x150 초과·5,000B 이상·비율 3:1 이내) 충족. 네이버 봇(Yeti) UA로 홈·게시판·칼럼·
  robots.txt·sitemap.xml 모두 200.
- **칼럼 대표 이미지는 150x150을 넘겨야 한다.** 칼럼 `씹을 때만 시큰한 치아, 신경치료 해야 하나요?`의
  대표 이미지가 100x100·4,506B라 og:image 규격에 미달해 본문 첫 이미지(1080x1080)로 교체했다.
  관리자에서 작은 썸네일을 대표 이미지로 지정하면 같은 문제가 재발한다.

### 네이버 서치어드바이저 — 등록·소유확인·사이트맵 제출 완료

- **2026-09-17 `https://egundc.com`을 서치어드바이저에 등록**했다(계정: 서울이건치과의원 / seoul2gun).
  그 전까지 사이트 목록이 비어 있어 네이버 쪽에는 수집 현황·노출 데이터 계기판이 없었다.
- **소유확인 완료.** 방법은 HTML 태그이며 코드는 `62dc81a6f5676827f6b1f38fd85a68bd8bc5978b`.
  `data/tracking.ts`의 `naverSiteVerification` 배열 **첫 번째**에 둔다. 같은 이름의 메타 태그가
  여러 개일 때 확인 크롤러가 첫 번째만 읽는 경우가 있어 순서가 중요하다. 기존 코드
  (`87980840c9...`)는 다른 계정의 확인이 풀리지 않도록 뒤에 남겨 둔다.
- **사이트맵 제출 완료** (`sitemap.xml`, 2026-09-18 16:24 등록). 입력란에는 **전체 URL**
  (`https://egundc.com/sitemap.xml`)을 넣어야 등록된다. `sitemap.xml`만 넣으면 오류 메시지 없이
  조용히 실패한다.
- 브라우저 자동화는 **`--account u1`** 로 접근한다(u0는 네이버 로그아웃 상태).
- **간단체크 도구는 캡차가 걸려 자동으로 재실행할 수 없다**(로그인 계정 기준 일간 10회 제한).
  도구가 지적했던 두 항목(`사이트 설명`·`Open Graph 설명`이 80자 초과)은 운영에서 직접 확인했고
  둘 다 54자로 기준을 충족한다. 소유확인·간단체크만 캡차를 요구하며 사이트맵 제출은 요구하지 않는다.
- 첫 소유확인 직후에는 노출/클릭·사이트 진단 리포트가 비어 있다. 생성까지 시간이 걸린다.

## 직전 상태 — 이미지 대체 텍스트·페이지 설명문 정리 완료

**2026-09-17 PR #4(병합 커밋 `a110549`)를 같은 배포 절차로 반영했다.**
운영 커밋은 `a110549`였으며, 직전 릴리스(`2ac7cee`)와 백업은 보존돼 있다.

| 릴리스 | 내용 |
|---|---|
| `11d7578` | 칼럼 이미지 설명 누락·빈 자리표시자 수정 |
| `649ef2d` | 사진 내용을 확인한 대체 텍스트 보강 |
| `514df72` | 짧았던 페이지 설명문 보강 |

- 칼럼 본문은 DB(Supabase `columns.content`)에 있어 **코드 배포와 무관하게 즉시 반영된다.**
  2026-09-17에 본문 이미지 63장에 설명을 넣고, 개발 서버 주소로 저장돼 깨져 있던 이미지 1장을
  상대경로로 고쳤으며, 회색 빈 상자로 보이던 자리표시자 블록 6개를 지웠다.
  작업 전 백업: `.omo/alt-batch/columns-backup-20260916-235051.json` (대상 칼럼 11개 전문).
- 칼럼 이미지 설명은 세 겹으로 보장한다. (1) 네이버 임포트 입력에서 빈 alt 속성을 넘기지 않음,
  (2) 프롬프트에 alt를 비우지 말라는 규칙, (3) 저장 직전 `fillMissingImageAlt`가 캡션·제목으로 채움.
  자리표시자 정리는 `removeUnuploadedImageBlocks`가 맡으며 `src=""`인 블록도 제거한다. 둘 다 `lib/column-ai.ts`에 있다.
- 진료 페이지 이미지 설명은 `types/treatment.ts`의 `imageAlt`·`bottomImageAlt`·`photoGrid[].alt`로 관리한다.
  값이 없으면 기존처럼 `title`·`caption`을 쓴다.
- 배포 검증: 두 도메인 스모크 `passed: true`(검사 173·자산 168·공개 칼럼 18), 사이트맵 37페이지 전수에서
  설명 빠진 alt 0건·약한 alt 0건·설명문 50자 미만 0건·h1 정확히 1개·빈 img 태그 0건.
  직전 배포분(robots Host 제거, IndexNow 키 200, www→apex 308, 구주소 308, 관리자 추적 미삽입)도 회귀 없음.
- 소아 페이지 이미지 3장(`child_treat.png`·`child-2.png`·`kids-2.png`)은 아이 얼굴이 식별 가능한
  상태로 노출되지만, **2026-09-17 소유자가 촬영·게시 동의를 받은 사진이라고 확인했다.** 교체 대상이 아니다.
- **미해결:** 칼럼 `앞니 라미네이트가 깨졌다면 다시 제작해야 할까요?`의 본문이 906KB이며 99%가
  base64로 인라인된 이미지 한 장(676KB)이다. 페이지 용량 개선이 필요하다.

## 직전 상태 — 검색 노출·측정 정리 반영 완료

**2026-09-16 PR #3(병합 커밋 `2ac7cee`)을 같은 배포 절차로 반영했다.**
운영 최신 커밋은 `2ac7cee`였으며, 직전 릴리스(`5d5f54e`)와 백업은 보존돼 있다.

| 릴리스 | 내용 |
|---|---|
| `d4ec014` | robots.txt `Host:` 제거, 구주소 23건·www 리다이렉트 |
| `96cb73e` | 이미지 대체 텍스트와 h1 구조 정리 |
| `11c1bac` | 관리자 화면 방문자 집계 제외 |
| `0a10f17` | IndexNow 키 배치와 칼럼 자동 통지 |

- 리다이렉트 규칙은 `next.config.ts`의 `redirects()` 한 곳에서 관리한다. Next의 `permanent: true`는 301이 아니라 **308**을 반환하며 구글은 둘을 동일하게 처리한다.
  구주소는 `/consult/reservation`→`/`, `/consult/*`→`/column`, `/intro/*`→`/about`, `/aesthetic/*`→`/digital-prosthesis` 네 갈래다.
- 방문자 집계 제외 경로는 `lib/tracking-paths.ts`의 `UNTRACKED_PREFIXES`에서 관리한다. 현재 `/admin`만 들어 있다.
  관리자 접속이 빠지므로 배포 직후 GA4·네이버 방문자 수치가 실제값으로 내려갈 수 있다.
- IndexNow 키는 `public/1b88a06921ae4f9eb42c1b810a62a4ec.txt`이고 통지 로직은 `lib/indexnow.ts`에 있다.
  **키 파일 이름과 내용이 같아야 하며 둘 중 하나만 바꾸면 검색엔진이 403으로 거절한다.**
  공개 상태로 저장된 칼럼만 통지하며, 통지 실패는 저장 결과에 영향을 주지 않는다.
- 배포 검증: 두 도메인 스모크 `passed: true`(검사 173·자산 168·공개 칼럼 18), 구주소 23/23 리다이렉트,
  `www.egundc.com` → apex 이동, robots.txt `Host:` 없음, 관리자 화면에 추적 스크립트 미삽입,
  사이트맵 37페이지 h1 정확히 1개, 비장식 이미지 alt 누락 0건, IndexNow 키 200.
  실제 브라우저로 계산 스타일을 비교해 바뀐 제목 요소의 크기·굵기·색·여백이 배포 전과 같음을 확인했다.
- 주의: `next lint`는 Next 16에서 제거돼 `npm run lint`가 실패한다. `eslint.config.*`도 없어 린트 실행 경로가 없다.
- 주의: 프로젝트 루트의 `.codegraph` 심볼링크 대상이 사라지면 `next build`가 `ENOENT: stat '.../.codegraph'`로 실패한다. 끊어진 링크를 지우면 통과한다.

## 직전 상태 — 측정·정리 후속 반영 완료

**2026-09-13 후속 릴리스 3건을 같은 배포 절차로 반영했다.**
운영 최신 커밋은 `5d5f54e`였으며, 직전 릴리스와 백업은 보존돼 있다.

| 릴리스 | 내용 |
|---|---|
| `d1bd42e` | 네이버 애널리틱스 설치 (경로 이동까지 집계) |
| `cab97c1` | 상담 신청·전화·카카오 전환 이벤트 |
| `5d5f54e` | 진료 안내 페이지 본문·의료법 고지 보강 |

- 추적 ID는 `data/tracking.ts` 한 곳에서 관리한다. 값이 비면 해당 스크립트는 렌더링되지 않는다.
  네이버 애널리틱스는 `wa` = `3ce350e52491c4`, 컴포넌트는 `components/seo/NaverAnalytics.tsx`.
- 전환 이벤트 이름은 `lib/analytics-events.ts`의 `consult_submit`·`phone_click`·`kakao_click`.
  GA4 보고서 설정과 연결되므로 **이름을 임의로 바꾸지 않는다.**
- 진료 안내 페이지 본문은 `lib/board-carousel.ts`의 `sections`에서 관리한다.
  치료 설명 페이지는 `requiresTreatmentNotice: true`로 의료법 고지를 노출한다.
- 서버 정리 완료: 디스크 여유 **16GB → 29GB**. 구 스크립트 백업·미활성 릴리스·종료된 업로드를 삭제했고
  사고 보존본, 현재·직전 릴리스, 최신 백업 2개, appuser 홈은 그대로 두었다.
- **미해결:** 관리자 비밀번호가 공개 저장소 이력의 값과 동일하다(아래 후속 조치 항목). 관리 주체 확인 후 교체 필요.
- Search Console에 `sitemap.xml`을 제출했으나 제출 직후 상태는 「가져올 수 없음」이다.
  사이트맵 자체는 Googlebot 200·XML 유효·37 URL·robots 허용으로 확인됐으므로 재제출 전 며칠 관찰한다.
