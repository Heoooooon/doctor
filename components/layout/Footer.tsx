...
export default function Footer() {
  ...
  return (
    <footer className="site-footer bg-[#0a0c0f] text-white/60 border-t border-white/10">
      <div className="site-footer-inner max-w-7xl mx-auto px-5 sm:px-6 lg:px-10 py-7 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-white/10">
          <div>
            ...
            <p className="footer-brand-en text-white/60 text-[18px] tracking-[0.18em] uppercase mt-1">
        ...
        {/* 사업자 정보 */}
        <div className="footer-meta flex flex-wrap items-center gap-x-3 gap-y-2 text-[18px] text-white/60 mt-6">
          <span><span className="text-white/50">대표</span> {clinicInfo.representative}</span>
          <span className="text-white/40" aria-hidden="true">|</span>
          <span><span className="text-white/50">사업자명</span> 서울이건치과의원</span>
          <span className="text-white/40" aria-hidden="true">|</span>
          <span><span className="text-white/50">사업자번호</span> {clinicInfo.businessNumber}</span>
          <span className="text-white/40" aria-hidden="true">|</span>
          <span>
          ...
          <span className="text-white/40" aria-hidden="true">|</span>
          <span><span className="text-white/50">FAX</span> {clinicInfo.fax}</span>
        </div>
        <p className="footer-address text-[18px] text-white/60 mt-2">
          <span className="text-white/50">주소</span> {clinicInfo.address}
        </p>

        {/* 면책 + 저작권 */}
        <p className="footer-disclaimer text-[18px] text-white/50 leading-relaxed mt-5">
          본 페이지의 내용은 의료법에 따라 참고용 정보이며, 정확한 진단과 치료는 내원 상담을 통해 결정됩니다.
        </p>
        <p className="footer-copyright text-[18px] text-white/50 mt-2 tracking-wide">
