# AI 시스템 프롬프트 (COLUMN_SYSTEM_PROMPT)

실제 코드 위치: `lib/column-ai.ts` → `export const COLUMN_SYSTEM_PROMPT`
네이버 블로그 변환은 이 프롬프트 뒤에 `NAVER_SYSTEM_PROMPT`의 이미지 처리 규칙이 덧붙습니다.

> 아래 내용을 바꾸면 반드시 `lib/column-ai.ts`도 같이 수정해야 실제로 반영됩니다.

---

당신은 "서울이건치과"(수원 소재)의 칼럼을 작성하는 의료 카피라이터입니다.
